...
function newWindow(link){
  ...
  newWindow.onload = function() {
    ...
    contentDiv.innerHTML = '<form id="dataForm"><input type="text" id="inputField"><button type="button" id="submitButton">Submit</button></form>';
    newWindow.document.body.appendChild(contentDiv);

    contentDiv.querySelector('#submitButton').addEventListener('click', function() {
...
function loadTagDatas(parentTag){
  $.get("/loadTags/" + parentTag.tagName, function(data) {
      data.forEach(e =>{
        let childTag = new Tab($('#' + parentTag.tagName + "Div_Body"),parentTag.tagName + "_" + e.tagName);
        ...
        $('#' + parentTag.tagName + "_" + e.tagName).on('dblclick', (e) => divDblClicked(e));
      })
  });
}
...
function divDblClicked(e){
  ...
  $('#' + e.target.id).focus();
  let parentTagName = $('#' + e.target.id).parent().siblings().children().first().text();
  ...
  $('#' + e.target.id).on('blur keypress',(e) => {
    if(e.type == 'blur' || (e.type == 'keypress && e.which ==13')){
      ...
      console.log("/editTagName/" + parentTagName + "-" + originalName + "-" + editedName);
      $.get("/editTagName/" + parentTagName + "-" + originalName + "-" + editedName, function(data) {
        console.log("실행됨");
      });
    }
  })
...
class Tab{
    ...
    appendTag(className){
      this.parentTag.append($('<div class = "' + className + '" id = "' + this.tagName + '">'));
    }
